...
class OrderController extends Controller
{
    ...
    public function order_upload(Request $request)
    {
        ...
        if (isset($img)) {
            ...
            if (strpos(__DIR__, 'MAMP') !== false) {
                ...
                copy($from, $to);
            } elseif (strpos(__DIR__, 'r2325683') !== false) {
                //お名前用
                $from = '/home/r2325683/fedex/public/storage/order/' . $file_name;
                $to = '/home/r2325683/ccmapp/public/storage/order/' . $file_name;
                copy($from, $to);

            }elseif (strpos(__DIR__, 'fedex-sakura') !== false) {
                    $image->img_path = 'storage/' . $dir . $file_name;
                    $from = '/Users/segawa/www/html/fedex-sakura/storage/app/public/order/' . $file_name;
                    $to = '/Users/segawa/www/html/ccmapp/storage/app/public/order/' . $file_name;
                    copy($from, $to);

            } else {
                $from = $_SERVER['DOCUMENT_ROOT'] . '/storage/order/' . $file_name;
                $to = str_replace('fedex', 'ccmapp', $from);
                copy($from, $to);
            }
    ...
    public function order_payment(Request $request)
    {
        ...
        $order->save();
            ...
            //$items = session()->get('items')->toArray();
            $items = session()->get('items');
            $ctn_total = session()->get('ctn_total');
            $quantity_total = session()->get('quantity_total');
            ...
            $content = [
                ...
                'phone' => $order->phone,
                'fax' => $order->fax,
                'bill_company_address_line1' => $order->bill_company_address_line1,
                'bill_company_address_line2' => $order->bill_company_address_line2,
                'bill_company_city' => $order->bill_company_city,
                'bill_company_state' => $order->bill_company_state,
                'bill_company_country' => $order->bill_company_country,
                'bill_company_zip' => $order->bill_company_zip,
                'bill_company_phone' => $order->bill_company_phone,
    ...
    public function payment_upload(Request $request)
    {
        ...
        if (isset($img)) {
            ...
            Mail::to($email)->bcc($bcc)->send(new CompleteMail($content));
    ...
    //マイページから送金書をアップするための画面へ。ここでもメール送信。
